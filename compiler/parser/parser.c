...
static TokenType idOrKeyword(const char* start, uint32_t length) {
    uint32_t idx = 0;
    struct KeywordToken token = keywordToken[idx];
    while (token.keyword != NULL) {
        if (token.length == length && memcmp(token.keyword, start, length) == 0) {
            return token.token;
        }
        idx++;
        token = keywordToken[idx];
    }
    return TOKEN_ID;
}
    ...
    while (true) {
        ...
        if (parser->curChar == '%') {
            ...
            if (parser->interpolationExpectRightParenNum > 0) {
                COMPILE_ERROR(parser, "sorry, I don't support nest interpokate expression");
            }
