...
static uint32_t hashValue(Value value) {
    switch (value.type)
    {
        ...
        default: 
            RUN_ERROR("unsupported type hash");
    }
    return 0;
}
...
static bool addEntry(Entry* entries, uint32_t capacity, Value key, Value value) {
    ...
    while (true) {
        ...
        if (valueIsEqual(entries[index].key, key)) {
            entries[index].value = value;
            return false;
        }
...
Value removeKey(VM* vm, ObjMap* objMap, Value key) {
    ...
    if (objMap->count == 0) {
        ...
        if (objMap->count > MIN_CAPACITY) {
            if (objMap->capacity / (CAPACITY_GROW_FACTOR) * MAP_LOAD_PERCENT > objMap->count) {
                uint32_t newCapacity = objMap->capacity / CAPACITY_GROW_FACTOR;
                if(newCapacity < MIN_CAPACITY) {
                    newCapacity = MIN_CAPACITY;
                }
